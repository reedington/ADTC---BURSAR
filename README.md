...
| Phase 2B | **Data & Evaluation** | 🟡 harness and 140 expansion drafts complete; human/Yoruba review and dataset freeze pending |
...
| Phase 3 | **Baselines / training / calibration** | 🟡 reproducible commands complete; four-model i5 run, rented-GPU training, and fitted artifact pending |

**Test suite:** 228 passing, 1 skipped (the tokenizer/model-bound check runs during the target
hardware session). Passing fake-backend tests validate mechanics and safety seams, not model
quality or performance. Benchmarks count only when measured on i5-class hardware at 4 threads.
...
bursa/            # the application
  ...
  importers/, normalize.py, matcher.py, candidates.py, pipeline.py, review.py
  inference/      # llama-server backend, GBNF grammar, prompt builder, token budget, validation
  features.py, calibrator.py                                  # versioned JSON logistic policy
bursa_eval/       # evaluation & data tooling (Agent D)
  ...
  harness/        # gold, ADTC/proxy, bare-model suites + scorecard; adapters.py = native chat templates
  dataset.py, authoring.py   # reviewed authoring gate, dual-format mixture, leak-free freeze
  baseline.py, training.py, calibration.py                    # C2/C3 workflows
  proxy.py, general.py       # pinned MMLU enterprise proxy + general-capability examples
  repro.py        # shared provenance helpers (checksums, toolchain, environment capture)
configs/          # pinned run configs: data/release, baseline/models, training, calibration, scorecard suites
data/gold/        # team-authored gold cases (YAML, one per file)
data/gold_drafts/ # 140 unapproved expansion drafts; never used for training until promoted
data/build/       # generated {train,val,test}.jsonl (git-ignored, regenerable from seed + gold)
docs/             # PRD.md, MODEL_ARCHITECTURE.md, PROJECT_CONTEXT.md, TRAINING_AND_BASELINES.md
  superpowers/specs/, superpowers/plans/, superpowers/runbooks/
  checklists/prerequisites.md, baselines/                     # C1 hardware gate + recorded baseline runs
tests/            # pytest + hypothesis
demo/             # fictional CSV fixtures
media/            # verified application screenshots
Dockerfile.training, requirements-training.lock.txt           # pinned rented-GPU training image
...
export BURSA_CALIBRATION_PATH=artifacts/calibration/final.json
...
| `bursa-data validate --config configs/data/release.json --require-final` | enforce 154 reviewed cases, language totals, 42 abstentions, and reviewer checksums |
| `bursa-data build ...` / `bursa-data freeze ...` | build and freeze the reproducible 70/30 training corpus |
| `bursa-baseline fetch --accept-third-party-terms` | fetch and verify the four pinned evaluation artifacts after the disk preflight |
| `bursa-baseline run ...` / `bursa-baseline compare ...` | produce the four-model table and qualifying C2 decision |
| `bursa-train run --model all ...` | run both pinned Unsloth QLoRA/export experiments on the rented GPU |
| `bursa-calibrate collect ...` / `fit ...` / `verify ...` | collect observable features and emit the portable JSON logistic policy |
| `bursa-scorecard run --suite gold\|enterprise\|bare\|stress\|all ...` | run an individual suite or the complete scorecard |
...
All money is **integer minor units** (no floats); allocations can't exceed the transaction; the ledger is **append-only** (reversals, never edits — enforced by DB triggers); the LLM never writes ledger events. Model proposals default to review and gain automatic-post authority only from a model-hash-bound calibration artifact that passed the zero-false-post gate; the server still rebuilds and validates every event.
...
- Agent D data foundation: [design](docs/superpowers/specs/2026-07-25-agent-d-data-foundation-design.md) · [plan](docs/superpowers/plans/2026-07-25-agent-d-data-foundation.md)
...
- Hardware gate: [C1 prerequisites checklist](docs/checklists/prerequisites.md) · [recorded baseline runs](docs/baselines/)
- Submission: [technical report](REPORT.md) · [demo script](docs/DEMO_SCRIPT.md) · [privacy](PRIVACY.md) · [security](SECURITY.md)
- Phase 3: [training, calibration, and four-model runbook](docs/TRAINING_AND_BASELINES.md)
